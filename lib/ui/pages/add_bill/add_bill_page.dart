...
import 'package:zhiliao/common/event_bus.dart';
...
import 'package:zhiliao/ui/widgets/input_textview_dialog.dart';
import 'package:zhiliao/ui/pages/add_bill/number_keyboard.dart';

class AddBillPage extends StatefulWidget {
  final BillRecordModel recordModel;

  const AddBillPage({Key key, this.recordModel}) : super(key: key);
  @override
  State<StatefulWidget> createState()=> AddBillPageState();

}

class AddBillPageState extends State<AddBillPage> with
  ...
  bool _isAdd = false;
  String ledger;
  List<DropdownMenuItem> ledgerMenu = List();
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        DropdownButton(
                          items: ledgerMenu,
                          hint:new Text("选择账单"),// 当没有初始值时显示
                          onChanged: (selectValue){//选中后的回调
                            setState(() {
                              ledger = selectValue;
                            });
                          },
                          value: ledger,// 设置初始值，要与列表中的value是相同的
                          elevation: 10,//设置阴影
  ...
  void _getLedger()async{
    ...
    for(int i = 0 ; i < ledgers.length ; i++) {
     DropdownMenuItem dropdownMenuItem = DropdownMenuItem(child: Text(ledgers[i].name),value: ledgers[i].ledgerId);
     ledgerMenu.add(dropdownMenuItem);
    }
  }
  ...
  void _record() {
    ...
    billRepository.addBill(ledgerId: ledger,money: _numberString,billType: _tabController.index.toString(),categorySort: (item.sort+1).toString());
  }
